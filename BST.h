...
public:
   ...
   ------------------------------------------------------------------------*/
    //function that prints an item that aldready exists in the tree
    void printInfo(const DataType&);//function definition on line 287
...
private:
    ...
    public:
        ...
        BinNode* left;
        BinNode* right;
        int count;// accounts for the number of times an item is inserted in the BST 
        ...
        BinNode(DataType item)
            : data(item), left(0), right(0), count(1) //intially construct count to 1 as the word appears at least once
        {}
...
inline void BST<DataType>::insert( DataType item)
{
    ...
    //if item aldready exists in the tree increment the count member of the BinNode that contains it as the word appears more than once
    else
        locptr->count++;
...
template<typename DataType>
inline void BST<DataType>::printInfo(const DataType& item)
{
    //traverse through the tree
    // 
    //pointers to BinNode's
    BinNodePointer locptr = myRoot;
        BinNodePointer parent = 0;
    bool found = false;
    while (!found && locptr != 0)
    {
        if (item < locptr->data)       // descend left
    ...
    //when item is found, output the data of the node 
    //item will always exist in the tree as this function is called under that condition
    cout << locptr->data;
    cout <<"Count:"<< locptr->count << endl;//output the count member of the node 
}
    ...
    BinNodePointer subtreeRoot) const
{
    if (subtreeRoot != 0)
    {
        ...
        out << "Count:" << subtreeRoot->count << endl;// V operation
        out << '\n';
